...
const SpecialistAdmin = () => {
  ...
  const fetchData = async () => {
    try {
      const response = await fetch('http://127.0.0.1:8000/api/spesialis/data');
  ...
  const columns = [
    ...
    },
    {
      ...
      type: 'actions',
      width: 150,
      renderCell: (params) => <SpecialistAction id={params.id} onSuccess={fetchData} />,
    },
  ];
    ...
    <Admin>
      ...
      >
        ...
        <Button
          ...
          sx={{ display: 'flex' }}
        >
          <Add sx={{ fontSize: { xs: 16, md: 20 }, color: '#ffffff' }} />
